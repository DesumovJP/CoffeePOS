...
export default factories.createCoreController('api::supplier.supplier', ({ strapi }) => ({
  async create(ctx) {
    ...
    await strapi.service('api::shift.shift').logCurrentShiftActivity('supplier_create', {
  ...
  async update(ctx) {
    ...
    const entry: any = response?.data || {};
    await strapi.service('api::shift.shift').logCurrentShiftActivity('supplier_update', {
      documentId: entry.documentId,
      name: entry.name,
      user: ctx.state?.user?.username || ctx.state?.user?.email,
    });
  ...
  async delete(ctx) {
    ...
    await strapi.service('api::shift.shift').logCurrentShiftActivity('supplier_delete', {
