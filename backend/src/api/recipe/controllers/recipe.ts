...
export default factories.createCoreController('api::recipe.recipe', ({ strapi }) => ({
  async create(ctx) {
    ...
    await strapi.service('api::shift.shift').logCurrentShiftActivity('recipe_create', {
  ...
  async update(ctx) {
    ...
    const entry: any = response?.data || {};
    await strapi.service('api::shift.shift').logCurrentShiftActivity('recipe_update', {
      documentId: entry.documentId,
      name: entry.name,
      user: ctx.state?.user?.username || ctx.state?.user?.email,
    });
  ...
  async delete(ctx) {
    ...
    await strapi.service('api::shift.shift').logCurrentShiftActivity('recipe_delete', {
